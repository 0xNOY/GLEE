...
class GLEE_Model(nn.Module):
    ...
    def __init__(
        ...
        self.lang_projection = nn.Parameter(
            torch.rand(cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM, cfg.MODEL.DIM_PROJ)
        )
        ...
        trunc_normal_(self.mask_sptial_embed[3], std=0.02)
    ...
    def forward(
        ...
        if self.text_encode_type == "clip_teacher":
            if task not in ["grounding", "rvos"]:
                ...
                if True:  # early_fusion
                    gather_all_classtoken = token_x.flatten(0, 1)[
                        tokenized["attention_mask"].flatten(0, 1) > 0
                    ]
                    gather_all_classtoken = gather_all_classtoken.unsqueeze(0).repeat(
                        len(images), 1, 1
                    )  # [bs,L,C]
                    gather_all_classtoken_mask = (
                        torch.ones_like(gather_all_classtoken[:, :, 0]) > 0
                    )  # [bs,L]
                    early_semantic = {
                        "hidden": gather_all_classtoken.float(),
        ...
        if "grounding" in prompts:
            if (
                ...
                or self.text_encode_type == "clip_teacher"
            ):
                ...
                lang_feat_pool = agg_lang_feat(
                    token_x, non_zero_query_mask, pool_type="average"
                ).unsqueeze(
                    1
                )  # (bs, 1, 768)
                ...
                ] = ~non_zero_query_mask.bool()  # [bz,len]
        ...
        if "spatial" in prompts:
            ...
            key_promptmasks = [
                m.unsqueeze(0) for m in prompts["spatial"]
            ]  # bz*[1,1,H,W]
        ...
        if "spatial" in prompts:
            ...
            non_zero_pos_point = [
                rand_sample(
                    (m.nonzero()[:, 1:] / divisor).t(), self.max_spatial_len[-1]
                ).t()
                for m in extra["spatial_query_pos_mask"]
            ]
            non_zero_pos_point = nn.utils.rnn.pad_sequence(
                non_zero_pos_point, padding_value=-1
            ).permute(1, 0, 2)
            non_zero_pos_mask = non_zero_pos_point.sum(dim=-1) < 0
            spatial_query_pos = point_sample(
                mask_features,
                non_zero_pos_point.flip(dims=(2,)).type(mask_features.dtype),
                align_corners=True,
            )  # [(N, C, P)
            spatial_query_pos = (
                torch.stack(
                    [
                        x[m].mean(dim=0, keepdim=True)
                        for x, m in zip(
                            spatial_query_pos.transpose(1, 2), ~non_zero_pos_mask
                        )
                    ]
                )
                .transpose(0, 1)
            ...
            non_zero_neg_point = [
                ...
                for m in extra["spatial_query_neg_mask"]
            ]
            non_zero_neg_point = nn.utils.rnn.pad_sequence(
                non_zero_neg_point, padding_value=-1
            ).permute(1, 0, 2)
            non_zero_neg_mask = non_zero_neg_point.sum(dim=-1) < 0
            spatial_query_neg = point_sample(
                mask_features,
                non_zero_neg_point.flip(dims=(2,)).type(mask_features.dtype),
                align_corners=True,
            )
            spatial_query_neg = (
                torch.stack(
                    [
                        x[m].mean(dim=0, keepdim=True)
                        for x, m in zip(
                            spatial_query_neg.transpose(1, 2), ~non_zero_neg_mask
                        )
                    ]
                )
                .transpose(0, 1)
            ...
            for i in range(len(multi_scale_features)):
                ...
                non_zero_query_point_pos = [
                    rand_sample(
                        (m.nonzero()[:, 1:] / divisor).t(), self.max_spatial_len[i]
                    ).t()
                    for m in extra["spatial_query_pos_mask"]
                ]
                ...
                pos_neg_indicator = [
                    torch.cat(
                        [
                            ...
                            -torch.ones(y.shape[0], device=y.device),
                        ]
                    )
                    for x, y in zip(non_zero_query_point_pos, non_zero_query_point_neg)
                ]
